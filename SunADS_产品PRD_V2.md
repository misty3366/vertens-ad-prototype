...
> 本文只描述 SunADS 当前确认的产品方向和最新功能。产品从单一建材家居门店扩展为跨行业线下门店，但仍聚焦同一个结果：用可规模化的本地内容带来预约、到店、咨询和成交。Token 中转、通用 API 平台或泛 AI 工具不属于本版本核心业务。
...
> **SunADS 是面向海外线下门店的 AI 到店营销内容与数据工作台。**
>
...
SunADS 不以“会生成图片和视频”作为最终价值，而以以下闭环作为产品价值：
...
- 为门店提供策略、脚本、本地化和视频生产的 SunADS 内部创意团队。

### 0.3 首批行业模板

SunADS 的底层工作流保持通用，但模板、示例、默认 CTA 和效果指标必须按行业呈现。首版覆盖以下 16 个行业：
...
AI 已经显著降低脚本、翻译、配音、数字人和视频生成成本，但“生成工具”本身容易同质化。SunADS 的机会不在于再做一个通用生成器，而在于把以下资产组合起来：
...
| SunADS 创意制作人 / Producer | 处理 Creative Service Brief、生成并提交样片 | 被分配项目的内容与工作流权限 |
...
Creative Service 是由 SunADS 创意团队代客户完成策略、脚本、本地化、数字人指导、AI 生成和最终剪辑的服务，不是另一个自助生成器。
...
> **每个活跃 Workspace 每月由 SunADS Creative 带来的 Qualified Leads。**
...
SunADS V2.1 的核心不是“提供最多的 AI 工具”，而是把一个跨行业但结果一致的门店获客过程产品化：
...
只要产品始终围绕这条闭环迭代，SunADS 就不是一个容易被复制的通用生成工具，而会逐步形成“行业模板、真实案例、工作流、客户品牌资产与效果数据”共同构成的壁垒。
